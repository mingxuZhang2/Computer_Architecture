...
#include <string.h>
#include <stdint.h>
#include <arpa/inet.h> // 包含 ntohl 函数
#include "shell.h"
...
uint32_t mem_read_32(uint32_t address)
{
    ...
    for (i = 0; i < MEM_NREGIONS; i++) {
        if (address >= MEM_REGIONS[i].start &&
            ...
            uint32_t addresses =                 (MEM_REGIONS[i].mem[offset+3] << 24) |
...
void cycle() {                                                
  ...
  INSTRUCTION_COUNT++;
  if(INSTRUCTION_COUNT>all_inst) RUN_BIT=FALSE;
}
...
void load_program(char *program_filename) {                   
  ...
  while (fread(&word, sizeof(uint32_t), 1, prog) == 1) {
      word = ntohl(word); // 转换字节顺序
      printf("load words is: %u\n",word);
